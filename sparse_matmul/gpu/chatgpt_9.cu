    ...
    int A_rows, int B_cols, float* temp_values, int* temp_flags)
{
    ...
    int idx = 0;
    for (int col = lane; col < B_cols; col += WARP_SIZE) {
        if (flags[col]) {
            int pos = atomicAdd(&idx, 1);  // all lanes race here, atomic avoids overwrite
            C_indices[out_start + pos] = col;
            C_data[out_start + pos] = values[col];
        }
    }
}
