 ...
 *
 * This kernel replaces the need for the Thrust library, avoiding compiler compatibility issues.
...
__global__ void exclusive_scan_kernel(const int* d_in, int* d_out, int n) {
    ...
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        for (int i = 1; i <= n; ++i) {
            int val = d_in[i - 1];
            d_out[i] = sum + val; // This is an inclusive scan, let's correct
            sum += val;
        }

        // Correct to exclusive scan
        sum = 0;
        for (int i = 0; i < n; i++) {
            int temp = d_in[i];
            d_out[i] = sum;
        ...
        d_out[n] = sum; // The last element of indptr is the total NNZ
    }
}
...
__global__ void spgemm_numeric_optimized(const int num_rows, const int b_cols,
    ...
    // A simple insertion sort is efficient for small `current_nnz` in local memory.
...
void spgemm_gpu(const CSRMatrix& A, const CSRMatrix& B, CSRMatrix& C) {
    ...
    // This is launched with a single block and a single thread.
    if (A.rows > 0) {
