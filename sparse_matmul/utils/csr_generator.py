...
def generate_and_save_csr(matrix_name, matrix, output_dir='.'):
    matrix = matrix.tocsr()
    np.savetxt(f'{output_dir}/{matrix_name}_indptr.txt', matrix.indptr, fmt='%d')
    np.savetxt(f'{output_dir}/{matrix_name}_indices.txt', matrix.indices, fmt='%d')
    np.savetxt(f'{output_dir}/{matrix_name}_data.txt', matrix.data, fmt='%.6f')
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='.', help='Output directory for matrix files (default: current directory)')
